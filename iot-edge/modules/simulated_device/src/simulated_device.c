...
static int GetBridgeData(char *lpFileName);
    ...
    if (user_data != NULL)
    {
        ...
        while (module_data->simulatedDeviceRunning)
        {
            ...
            else
            {
                ...
                {
                    char msgText[128];
					int temp = GetBridgeData(module_data->fileName);

					if (temp < 0) // means error
					{
						LogError("Failed to read bridge file");
						Map_Destroy(newProperties);
						return 0;
					}
					avgTemperature = (double)temp;
                    ...
                    if (sprintf_s(msgText, sizeof(msgText), "{\"temperature\": %.2f}", avgTemperature) < 0)
                    {
                        LogError("Failed to set message text");
                    }
                    else
                    {
                        (void)printf("Device: %s %s, Temperature: %.2f\r\n",
